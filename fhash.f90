...
module fhash
    ...
    contains
        ...
        subroutine pop_node(this, node)
            class(fhash_list), intent(inout) :: this
            type(fhash_list_node), intent(out) :: node
            type(fhash_list_node), pointer :: old
            ...
            old => this%footer
            node%key = old%key
            if (allocated(old%value)) node%value = old%value
            node%error = .false.
            node%next => null()
            node%previous => old%previous
            this%footer => old%previous

            if (associated(this%footer)) then
                nullify(this%footer%next)
            else
                nullify(this%header)
            end if
